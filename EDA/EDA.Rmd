...
```{r}
sa_cons%>% plot_acf_diagnostics(.date_var = DATETIME,.value = TOTALDEMAND,.lags = 0:17520)
```
Plot seasonal diagnostics

```{r}
sa_cons %>% plot_seasonal_diagnostics(DATETIME, TOTALDEMAND,.feature_set = c("quarter","year","month.lbl"),.y_lab = "DEMAND")
```
```{r}
sa_cons %>% plot_seasonal_diagnostics(DATETIME, TOTALDEMAND,.feature_set = c("hour","wday.lbl","week"),.y_lab = "DEMAND")
